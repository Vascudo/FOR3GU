...
for b in range(0, 5):
    computer[b] = pygame.draw.rect(window, colors[b], pygame.Rect(box_position, 30, 60, 60))
    player[b] = pygame.draw.rect(window, colors2[b], pygame.Rect(box_position, 120, 60, 60))
    box_position += 90
...
while running:
    ...
    if event.type == pygame.QUIT:
        ...
        if player[0].collidepoint(event.pos):

            throw = Dice()
            player_array[4] = throw.throw()
            label = my_font.render(str(player_array[0]), 1, WHITE)
            window.blit(label, (330, 140))
            pygame.display.update()
